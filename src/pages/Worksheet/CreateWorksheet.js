...
import { addNewItem as onAddItem, updateItem as onUpdateItem } from "store/worksheet/actions"
...
const CreateSelfTest = ({ visible, isEdit, examBoardData = {}, onClose }) => {
  ...
  const {
    ...
    create_loading: pending(state, "ADD_WORKSHEET"),
    create_error: rejected(state, "ADD_WORKSHEET"),
    create_success: fulfilled(state, "ADD_WORKSHEET"),
    done: done(state, "ADD_WORKSHEET"),
    update_loading: pending(state, "UPDATE_WORKSHEET"),
    update_error: rejected(state, "UPDATE_WORKSHEET"),
    update_success: fulfilled(state, "UPDATE_WORKSHEET"),
    update_done: done(state, "UPDATE_WORKSHEET")
  }))
  ...
  useEffect(() => {
    if (visible) {
      dispatch(clean("ADD_WORKSHEET"))
      dispatch(clean("UPDATE_WORKSHEET"))
      if (!isEdit) {
        setSelftest({})
      }
    }
  ...
  useEffect(() => {
    if (create_success) {
      onClose()
      showToastMessage("success", "Worksheet successfully created")
      setSelftest({})
      dispatch(clean("ADD_WORKSHEET"))
    }
    if (update_success) {
      onClose()
      showToastMessage("success", "Worksheet successfully updated")
      dispatch(clean("UPDATE_WORKSHEET"))
    }
          ...
          {(create_success || update_success) && (
            <Col>
              <Alert color="success" role="alert">
                {isEdit
                  ? "Work Sheet Successfully updated"
                  : "Work Sheet Successfully created"}
              </Alert>
            </Col>
          )}
