...
import { Alert, Button, Col, Input, Modal, ModalBody, ModalHeader, Row } from "reactstrap"
...
import { addNewItem as onAddQuestion, updateItem as onUpdateQuestion } from "store/worksheet/questions/actions"
...
const CreateWorksheetQuestion = ({
  ...
  const {
    ...
    loading: pending(state, "ADD_WORKSHEET_QUESTIONS"),
    error: rejected(state, "ADD_WORKSHEET_QUESTIONS"),
    success: fulfilled(state, "ADD_WORKSHEET_QUESTIONS"),
    done: done(state, "ADD_WORKSHEET_QUESTIONS"),
    update_loading: pending(state, "UPDATE_WORKSHEET_QUESTIONS"),
    update_error: rejected(state, "UPDATE_WORKSHEET_QUESTIONS"),
    update_success: fulfilled(state, "UPDATE_WORKSHEET_QUESTIONS"),
    update_done: done(state, "UPDATE_WORKSHEET_QUESTIONS")
  }))
  ...
  useEffect(() => {
    if (visible) {
      dispatch(clean("ADD_WORKSHEET_QUESTIONS"))
      dispatch(clean("UPDATE_WORKSHEET_QUESTIONS"))
      if (!isEdit) {
        setSelftest({})
      }
    }
  ...
  const QuestionTypeHandler = (e) => {
    ...
    if (value == "custom") {
      setSelftest({ ...selftest, question_type: value, correct_answers: [] })
    } else {
      setSelftest({ ...selftest, question_type: value })
    }
  }

  const onSetSolution = (value) => {
    setSelftest({ ...selftest, solution: value })
  }

  return (
    <Modal isOpen={visible} toggle={onClose} size="xl">
      <ModalHeader toggle={onClose} tag="h4">
        {!!isEdit ? "Edit Worksheet Question" : "Add Worksheet Question"}
      </ModalHeader>
      <ModalBody>
        <AvForm
        ...
        >
          <Row>
            ...
            {/* MCQ Question Begin */}
            {selftest.question_type == "mcq" && (
              <>

                <Col>
                  <label>Answers</label>
                  <AnswerManage answers={selftest.answers} onChange={(answers) => {
            ...
            <Col xl="12" lg="12" md="12">
              <div className="mb-3">
                ...
                <UploadVideo onVideoUpload={onVideoUpload} editData={selftest.video_explanation} visible
          ...
          {(success || update_success) && (
            <Col>
              <Alert color="success" role="alert">
                {isEdit
                  ? "Work Sheet Question Successfully updated"
                  : "Work Sheet Question Successfully created"}
              </Alert>
            </Col>
          )}
...
export default withRouter(CreateWorksheetQuestion)
